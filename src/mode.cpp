...
//         reply(cli, _servname + " " + itos(ERR_NOSUCHNICK) + " " + cli.getNick() + " " + nick + " :No such nick\r\n");
//         return;
//     }
//     else if (!chanit->isUserInChannel(nick)) {
//         reply(cli, _servname + " " + itos(ERR_USERNOTINCHANNEL) + " " + cli.getNick() + " " + chName + " :they are not on that channel\r\n");
//         return;
//     }
//     else {
...
//     }
    ...
    // if (_params.size() < 2) {
    //     reply(client, _servname + " " + itos(ERR_NEEDMOREPARAMS) + " " + client.getNick() + " MODE :Not enough parameters\r\n");
    //     return;
    // }

    // if(_params.size() == 2) {
    //     reply(client, _servname + " " + itos(RPL_CHANNELMODEIS) + " " + client.getNick() +  " " + channelName + chanit->channelModeIs());
    //     return;
...
//                     } else {
//                         reply(client, _servname + " " + itos(ERR_NEEDMOREPARAMS) + " " + client.getNick() + " :Not enough parameters for mode +l\r\n");
//                         return;
//                     }
//                 } else {
...
//                     } else {
//                         reply(client, _servname + " " + itos(ERR_NEEDMOREPARAMS) + " " + client.getNick() + " :Not enough parameters for mode +k\r\n");
//                         return;
//                     }
//                 } else {
...
//                 } else {
//                     reply(client, _servname + " " + itos(ERR_NEEDMOREPARAMS) + " " + client.getNick() + " :Not enough parameters for mode +o/-o\r\n");
//                     return;
//                 }
//                 break;
//             default:
//                 reply(client, _servname + " " + itos(ERR_UNKNOWNMODE) + " " + client.getNick() + " " + mode + " :Unknown mode\r\n");
//                 return;
//         }
//     }
// }